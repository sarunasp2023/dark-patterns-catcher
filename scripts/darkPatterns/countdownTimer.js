                ...
                mutations.forEach((mutation) => {
                        if (mutation.type === 'characterData') {
                                if (/\d+/g.test(mutation.oldValue) && /\d+/g.test(mutation.target.textContent)) {
                                        ...
                                        if (oldValueDigits !== null && newValueDigits !== null && Number(oldValueDigits[oldValueDigits.length - 1]) - Number(newValueDigits[newValueDigits.length - 1]) === 1) {
                                                foundDarkPatterns++;
                                                if (highlight) {
                                                  mutation.target.parentElement.parentElement.classList.add('highlighted-dark-pattern');
                                                }
                                                if (modify) {
                                                  mutation.target.parentElement.parentElement.classList.add('modified-dark-pattern');

                                                  var childElements = mutation.target.parentElement.parentElement.children;
