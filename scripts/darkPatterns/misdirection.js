const ENGLISH_KEYWORDS = ["essential", "necessary", "only", "more", "decline", "settings", "reject", "refuse"];
...
function detectMisdirectionCases(highlight, modify, body) {
  ...
  const tagsWithoutChildTags = Array.from(body.querySelectorAll('div, p, span'))
  ...
  for (const tag of tagsWithoutChildTags) {
      ...
      if (buttons.length > 1) {
          ...
          const lastButtonBackgroundColor = getComputedStyle(buttons[buttons.length - 1]);
          const lastButtonPosition = buttons[buttons.length - 1].getBoundingClientRect();
          ...
          buttons.forEach(button => {
            ...
            const colorDifferenceFirst = deltaE(firstButtonBackgroundColor['background-color'], buttonBackgroundColor['background-color']);
            if(areElementsClose(firstButtonPosition, buttonPosition)){
              if (colorDifferenceFirst > DELTAE_COLOR_THRESHOLD) {
                const isGoodCookieFirst = hasGoodCookieKeyword(button.innerText.toLowerCase());
            ...
            if(areElementsClose(lastButtonPosition, buttonPosition)) {
              const colorDifferenceLast = deltaE(lastButtonBackgroundColor['background-color'], buttonBackgroundColor['background-color']);
              if (colorDifferenceLast > DELTAE_COLOR_THRESHOLD) {
                const isGoodCookieLast = hasGoodCookieKeyword(button.innerText.toLowerCase());
                if (isGoodCookieLast) {
                  goodCokiesButtonsList.push(buttons);
...
function modifyButtonStyles(buttonCase) {
  if(goodCokiesButtonsList){
    ...
    otherButtonsList.forEach(buttons => {
      if (buttons.length > 1) {
        const buttonIndex = buttonCase === "firstButtonCase" ? 0 : buttons.length - 1;
...
function areElementsClose(firstElement, secondElement) {
  ...
  var distanceSquared = Math.pow(firstElementX - secondElementX, 2) + Math.pow(firstElementY - secondElementY, 2);
