const scarcityPatternEng =
  /\d+\s*(?:\%|pieces?|pcs\.?|pc\.?|ct\.?|items?)?\s*(?:available|sold|claimed|redeemed)|(?:last|final)\s*(?:article|item)/i;
const scarcityPatternLt =
  /(?:sandėlyje)?(?:liko|likutis)?\s*(?:tik|mažiau)?(?:nei)?:?\s*[<>]?\s*\d+\+\s*(vnt\.)?/i;
...
function detectScarcityCases(highlight, modify, domCopy) {
  ...
  elements.forEach((element) => {
    if (
      ...
      (scarcityPatternEng.test(element.innerText) ||
        scarcityPatternLt.test(element.innerText))
    ) {
      darkPatternsCount++;
