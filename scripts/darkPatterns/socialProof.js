const socialProofPatternEng = /\d*\s*(?:other)?\s*(?:customers?|clients?|buyers?|users?|shoppers?|purchasers?|people)?\s*(?:have\s+|just\s+)\s*(?:(?:also\s*)?(?:bought|purchased|ordered)|(?:rated|reviewed))\s*(?:this|the\s*following)?\s*(?:product|article|item)?s?/i;
const socialProofPatternLt = /(?:(?:iš)?parduotas?|įsigijo|įsigytas|(?:nu|nusi)pirko)\s*\d*\s*(?:kart(?:ą|ų|us)|pirkėj(?:as|ų|ai))?/i;
...
function detectSocialProofCases(highlight, modify, domCopy){
  ...
  elements.forEach(element => {
    if (textTags.includes(element.tagName.toLowerCase()) && 
      (socialProofPatternEng.test(element.innerText) || socialProofPatternLt.test(element.innerText))) {
      darkPatternsCount++;
