document.addEventListener("DOMContentLoaded", async function () {
  ...
  const highlightPatternAlert = document.getElementById(
    "highlightPatternAlert"
  );
...
async function updateSettingsAndSendMessage() {
  try {
    ...
    document.getElementById("darkPatternsCount").textContent =
      response.darkPatternsCount;

    if (response.darkPatternsCount > 0) {
      document.getElementById("found-pattern-cases").textContent =
        "Found types";
    }

    if (countdownTimerCount > 0) {
      document.getElementById("countdownTimerCount").textContent =
        countdownTimerCount;
      const countdownTimerSection = document.getElementById(
    ...
    if (preselectionCount > 0) {
      document.getElementById("preselectionCount").textContent =
        preselectionCount;
      const preselectionSection = document.getElementById(
        "preselectionSection"
      );
    ...
    if (misdirectionCount > 0) {
      document.getElementById("misdirectionCount").textContent =
        misdirectionCount;
      const misdirectionSection = document.getElementById(
        "misdirectionSection"
      );
    ...
    if (disguisedAdCount > 0) {
      document.getElementById("disguisedAdCount").textContent =
        disguisedAdCount;
    ...
    if (socialProofCount > 0) {
      document.getElementById("socialProofCount").textContent =
        socialProofCount;
...
function loadSettings(highlightCheckbox, modifyCheckbox) {
  chrome.storage.local.get(
    ["highlightSetting", "modifySetting"],
    function (result) {
